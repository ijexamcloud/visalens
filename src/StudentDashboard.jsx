/**
 * VisaLens — Student Dashboard  v6.0
 * ─────────────────────────────────────────────────────────────────────────
 * New in v6:
 *   • Intent-Type Actions    — NBA band is clickable when intent_type==='email';
 *                              fires the existing mailto deeplink with action text
 *                              injected into the body, turning AI into a workflow engine.
 *   • Drag-and-Drop Kanban  — @dnd-kit/core; dragging a card across columns fires
 *                              handleStatusChange automatically. StatusSelect dropdown
 *                              still works as fallback.
 *   • Time in Stage         — status_updated_at column tracked client-side; Kanban
 *                              cards show a micro-indicator "Xd in stage".
 *   • Morning Brief         — "Brief me" button takes top-N urgent cases, sends to
 *                              Gemini via /api/cases/morning-brief, shows a modal
 *                              with a spoken-style paragraph summary.
 *   • Cache Invalidation    — _nbaCache.delete() on every mutation (v5.1 fix).
 *
 * Carried from v5:
 *   Smart Next Best Action, Board-first, Quick-Log Notes, Activity Ledger,
 *   Next Milestone, Kanban Board, One-Click Comms, Bulk Operations,
 *   Quick-Peek Drawer, Stale/Urgency, Doc Health, Skeletons,
 *   Smart Filters, Insight Panels.
 */
...
const FUNNEL_STAGES = [
  ...
  { key: 'applied',  label: 'Applied',     statuses: ['Application Paid', 'Application Submitted'],          color: '#B45309', bg: 'rgba(180,83,9,.1)'     },
...
/* ─── Helpers ────────────────────────────────────────────────────────── */
const scoreBand = s =>
  s >= 70 ? { label:'Strong',   cls:'sdb-ok',   color:'#059669' } :
  s >= 45 ? { label:'Moderate', cls:'sdb-warn', color:'#B45309' } :
            { label:'Weak',     cls:'sdb-err',  color:'#DC2626' };
...
/* ─── Supabase ───────────────────────────────────────────────────────── */
const COLS = 'id,case_serial,created_at,updated_at,status_updated_at,student_name,counsellor_name,overall_score,target_country,lead_status,expiry_date,expiry_doc_type,application_targets,notes';
const CONTACT_COLS = 'id,phone,email';
...
const _nbaCache = new Map(); // caseId → { action, rationale, priority }
...
// Entry format: [DD Mon HH:MM] text
...
const STATUS_STYLE = {
  ...
  'Follow up':             { bg:'var(--soft-orange-bg)', color:'var(--soft-orange-txt)' },
...
function ExpiryBadge({ expiryDate, expiryDocType }) {
  ...
  const color = d<=14?'#DC2626':d<=30?'#B45309':'#059669';
  const bg    = d<=14?'rgba(220,38,38,.1)':d<=30?'rgba(180,83,9,.1)':'rgba(5,150,105,.1)';
  return (
...
function DocHealthBar({ expiryDate, expiryDocType }) {
  ...
  const color = d<=0?'#DC2626':d<=14?'#DC2626':d<=30?'#B45309':'#059669';
...
/* ════════════════════════════════════════════════════════════════════════
   KANBAN BOARD — drag-and-drop via @dnd-kit/core
   Dragging a card to another column fires onStatusChange(caseId, newStatus)
   using the first status in the destination stage's statuses array.
   StatusSelect dropdown still works as a fallback.
════════════════════════════════════════════════════════════════════════ */
        ...
        <div style={{ display:'flex', alignItems:'flex-start', gap:7, marginBottom:8 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontSize:'var(--text-sm)', fontWeight:600, color:'var(--t1)', fontFamily:'var(--fh)', lineHeight:1.3, display:'flex', alignItems:'center', gap:4, flexWrap:'wrap' }}>
              {c.studentName}
              {hasAlert&&<span style={{ width:6, height:6, borderRadius:'50%', background:'#B45309', display:'inline-block' }} title="Policy alert"/>}
            </div>
            {c.counsellorName&&<div style={{ fontSize:10, color:'var(--t3)', fontFamily:'var(--fu)', marginTop:1 }}>{c.counsellorName}</div>}
          </div>
        </div>
        <div style={{ display:'flex', flexWrap:'wrap', gap:4, marginBottom:8 }}>
          <span className={`sdb-score-badge ${sb.cls}`} style={{ fontSize:10 }}>{c.overallScore}/100</span>
...
function KanbanBoard({ cases, alertCountries, savingId, onStatusChange, onPeek }) {
  ...
  function handleDragEnd({ active, over }) {
    ...
    // Map column → the canonical "entry" status for that column
...
/* ════════════════════════════════════════════════════════════════════════
   BULK ACTION BAR  — fixed bottom floating bar
════════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════════
   QUICK-PEEK DRAWER  (v3: + One-Click Comms section)
════════════════════════════════════════════════════════════════════════ */
function QuickPeekDrawer({ student, onClose, onOpenFull, onStatusChange, onNoteUpdate, savingId, policyAlerts, inboxAlerts=[] }) {
  ...
  const [activeTab,  setActiveTab]  = useState('info'); // 'info' | 'log'
  const [nba,        setNba]        = useState(null);   // { action, rationale, priority }
  ...
  useEffect(()=>{
    if (!student?.id) return;
    // Check cache first — avoids redundant re-fetches on re-open.
    // Cache is deleted by handleStatusChange / handleNoteUpdate on any mutation,
    // so a status change while the drawer is open will fall through to a fresh fetch.
  ...
  async function handleLogSave() {
    ...
    if (updated !== null) {
      ...
      setLogText('');
    // Bubble up so the parent row reflects updated notes + updated_at
  ...
  return (
    <>
      ...
      <div style={{ position:'fixed', top:0, right:0, bottom:0, width:'min(440px,92vw)', zIndex:301, background:'var(--s1)', borderLeft:'1px solid var(--bd)', boxShadow:'var(--sh3)', display:'flex', flexDirection:'column', animation:'sdb-slide-in-right .2s var(--eout)' }}>
        ...
        <div style={{ padding:'18px 20px 14px', borderBottom:'1px solid var(--bd)', background:'var(--s2)', display:'flex', alignItems:'flex-start', gap:12, flexShrink:0 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
              ...
              {stale&&<StaleTag/>}
              {hasAlert&&(
                <span style={{ fontSize:10, fontWeight:700, padding:'1px 6px', borderRadius:4, background:'rgba(180,83,9,.12)', color:'#B45309', fontFamily:'var(--fu)', letterSpacing:'.04em', textTransform:'uppercase', border:'1px solid rgba(180,83,9,.2)', display:'flex', alignItems:'center', gap:3 }}>
                  <Bell size={8}/> Alert
                </span>
              )}
            </div>
        ...
        {/* Next Best Action band */}
        {(nba || nbaLoading) && (() => {
          const NBA_COLORS = {
            urgent: { bg:'rgba(220,38,38,.08)', border:'rgba(220,38,38,.2)', text:'#DC2626', dot:'#DC2626' },
            high:   { bg:'rgba(249,115,22,.08)', border:'rgba(249,115,22,.2)', text:'#EA580C', dot:'#F97316' },
          ...
          // Build the mailto deeplink injecting the AI action as the email body
          ...
          const bandStyle = {
            ...
            flexShrink: 0,
            cursor: isEmailIntent ? 'pointer' : 'default',
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'0 0 8px' }}>
          ...
          {activeTab === 'info' ? (<>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', borderBottom:'1px solid var(--bd)' }}>
              ...
              <div style={{ padding:'14px 20px' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  <span className={`sdb-score-badge ${sb.cls}`} style={{ marginLeft:'auto' }}>{sb.label}</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ padding:'16px 20px', display:'flex', flexDirection:'column', gap:12 }}>
              ...
              {hasAlert&&(
                <div style={{ padding:'10px 12px', borderRadius:8, background:'rgba(180,83,9,.07)', border:'1px solid rgba(180,83,9,.2)', display:'flex', gap:8, alignItems:'flex-start' }}>
                  <Bell size={13} color="#B45309" style={{ flexShrink:0, marginTop:1 }}/>
                  <div>
                    <div style={{ fontSize:'var(--text-xs)', fontWeight:700, color:'#B45309', marginBottom:2 }}>Policy Alert Active</div>
            ...
            {targets.length>0&&(
              <div style={{ padding:'0 20px 16px' }}>
                <div style={{ fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'.07em', color:'var(--t3)', fontFamily:'var(--fu)', marginBottom:10 }}>Target Programs ({targets.length})</div>
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  {targets.slice(0,5).map((t,i)=>(
                    <div key={i} style={{ padding:'8px 10px', borderRadius:8, background:'var(--s2)', border:'1px solid var(--bd)', fontSize:'var(--text-xs)', color:'var(--t2)', fontFamily:'var(--fu)' }}>
                      <div style={{ fontWeight:600, color:'var(--t1)', marginBottom:2 }}>{t.university||t.institution||'University'}</div>
                      <div style={{ color:'var(--t3)' }}>{[t.program,t.intake_date||t.intake].filter(Boolean).join(' · ')}</div>
                    </div>
                  ))}
                  {targets.length>5&&<div style={{ fontSize:'var(--text-xs)', color:'var(--t3)', textAlign:'center', padding:'4px 0' }}>+{targets.length-5} more</div>}
                </div>
              </div>
            )}
...
export default function StudentDashboard({ onLoad, totalCases: totalCasesProp, lastSaved, orgSession, orgCredits, policyAlerts=[], inboxAlerts=[] }) {
  ...
  const handleStatusChange = useCallback(async(caseId,newStatus)=>{
    ...
    // Optimistic UI update — also stamp statusUpdatedAt for time-in-stage
    ...
    // Invalidate stale NBA cache — the action the AI recommended may now be done
  ...
  const handleNoteUpdate = useCallback((caseId, updatedNotes)=>{
    ...
    // Invalidate NBA cache — new notes may change the recommended action
    _nbaCache.delete(caseId);
  },[]);

  const handleMorningBrief = useCallback(async () => {
    setBriefOpen(true);
    if (briefText) return; // already fetched this session
    setBriefLoading(true);
    // Send top 8 urgent cases to the worker
  ...
  const handleBulkStatus = useCallback(async ns=>{
    ...
    setCases(prev=>prev.map(c=>ids.includes(c.id)?{...c,leadStatus:ns,updatedAt:ts}:c));
      ...
      <div className="sdb-root">
        ...
        <div className="sdb-command-bar">
          <div className="sdb-stats-row">
            ...
            <StatCard val={stats.alerted} label="Policy matches"   sub={`${policyAlerts.length} active alert${policyAlerts.length!==1?'s':''}`} color={stats.alerted>0?'#B45309':'var(--t3)'} onClick={()=>setActivePanel('risk')}/>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', gap:6, marginLeft:'auto' }}>
              ...
              {actionQueue.length > 0 && (
                <button onClick={handleMorningBrief} disabled={briefLoading}
                  style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 11px', borderRadius:8, border:'1px solid var(--bd)', background:'var(--s2)', color:'var(--t2)', cursor:'pointer', fontSize:'var(--text-xs)', fontWeight:600, fontFamily:'var(--fu)', transition:'all var(--fast)' }}
                  ...
                  title={`Summarise your ${Math.min(actionQueue.length,8)} most urgent cases`}
                >
                  {briefLoading?<Loader2 size={12} style={{ animation:'spin .7s linear infinite' }}/>:<Coffee size={12}/>}
                  Brief me
                </button>
              )}
              {/* List / Board toggle */}
          ...
          <div className="sdb-urgency-row">
            ...
            {stats.expiring>0&&<button className="sdb-pill sdb-pill--warn" onClick={()=>{setExpiryFilter('Soon');setActivePanel('expiry');}}><Clock size={11}/>{stats.expiring} expiring ≤30d</button>}
            ...
            {stats.alerted>0&&<button className="sdb-pill sdb-pill--warn" onClick={()=>setActivePanel('risk')}><Bell size={11}/>{stats.alerted} policy matches</button>}
          </div>
        </div>
        ...
        <div className="sdb-filters">
          ...
          {filtersActive&&<button className="sdb-clear-btn" onClick={clearFilters}><X size={12}/>Clear</button>}
            ...
            : filtered.length===0 ? (
              ...
              return (
                ...
                >
                  ...
                  <div><span className={`sdb-score-badge ${sb.cls}`}>{c.overallScore}/100</span></div>
        ...
        <div className="sdb-panels-row">
          ...
          <InsightPanel id="expiry" active={activePanel==='expiry'} onToggle={id=>setActivePanel(activePanel===id?null:id)} icon={<Clock size={14}/>} title="Expiry radar"
            badge={expiryRadar.filter(c=>c.days!==null&&c.days<=30).length>0?{label:expiryRadar.filter(c=>c.days!==null&&c.days<=30).length,urgent:expiryRadar.some(c=>c.days!==null&&c.days<=14)}:null} accent="#DC2626">
  ...
  return (
    <div className={`sdb-insight-panel${active?' sdb-insight-panel--open':''}`}>
      <button className="sdb-insight-hdr" onClick={()=>onToggle(id)}>
        ...
        {badge&&<span className="sdb-insight-badge" style={{ background:badge.urgent?'rgba(220,38,38,.1)':'rgba(29,107,232,.1)', color:badge.urgent?'#DC2626':'#1D6BE8' }}>{badge.label}</span>}
