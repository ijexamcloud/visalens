...
-- (Step 8 below) — NOT by the React client. days_in_stage is a computed column
-- that updates automatically as time passes even before exited_stage_at is set.
-- ───────────────────────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS funnel_stage_entries (
  ...
  -- Computed: live elapsed days. NULL safe — uses now() until case exits stage.
  days_in_stage     integer GENERATED ALWAYS AS (
    EXTRACT(DAY FROM (COALESCE(exited_stage_at, now()) - entered_stage_at))::integer
  ) STORED,
...
BEGIN
  ...
  -- Close the current open entry for this case
